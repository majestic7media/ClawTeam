<h1 align="center"><img src="assets/icon.png" alt="" width="64" style="vertical-align: middle;">&nbsp; OpenHarness: 에이전트 스웜 인텔리전스</h1>

<p align="center">
  <strong>AI 에이전트의 진화 🚀: 솔로 🤖 → 스웜 🦞🤖🤖🤖<br>
  OpenHarness은 AI 에이전트가 무리를 이뤄 함께 생각하고 일하며 더 빠르게 결과를 내도록 돕습니다</strong>
</p>
...
**2026-03-18** OpenHarness 프로젝트가 공개 출시되었습니다.

**2026-03-23** OpenHarness `v0.2.0`이 오늘 릴리스되었습니다.
...
OpenHarness은 **에이전트 스웜 인텔리전스**를 구현합니다. AI 에이전트가 스스로 협업 팀을 만들고, 복잡한 작업을 똑똑하게 쪼개고, 인사이트를 실시간으로 공유하면서 더 나은 해답으로 수렴하게 합니다.
...
OpenHarness에는 `skills/openharness/`에 재사용 가능한 skill이 들어 있습니다.

**Claude Code**

이 skill을 `~/.claude/skills/openharness`에 설치한 뒤, 이렇게 프롬프트를 주면 됩니다.

```
"웹 앱을 만들어줘. 작업은 openharness로 여러 에이전트에게 나눠서 진행해."
```

**Codex**

같은 skill을 `$CODEX_HOME/skills/openharness`(보통 `~/.codex/skills/openharness`)에 설치한 뒤, 이렇게 요청하면 됩니다.

```
이 작업을 여러 에이전트 팀으로 나누고 끝까지 조율하도록 $openharness을 사용해줘.
```
...
OpenHarness은 셸 명령을 실행할 수 있는 **어떤 CLI 에이전트**와도 함께 쓸 수 있습니다.
...
즉 **어떤 CLI 에이전트든** OpenHarness 팀에 참여할 수 있습니다. 셸 명령만 실행할 수 있으면 됩니다. 별도 SDK도, API 통합도, 프레임워크 종속도 필요 없습니다.
...
|---------|---------|---------|-------------|
| `data_dir` | `OH_DATA_DIR` | `~/.openharness` | 데이터 디렉터리 |
...
모든 상태는 `~/.openharness/` 아래 JSON 파일로 저장됩니다. 데이터베이스도, 서버도, 클라우드도 필요 없습니다. `tmp + rename` 방식의 원자적 쓰기로 충돌과 크래시에도 안전합니다.
...
기여를 환영합니다. OpenHarness은 확장 가능성을 염두에 두고 설계되었습니다.
...
- [Claude Code](https://claude.ai/claude-code)와 [Codex](https://openai.com/codex): OpenHarness의 팀원으로 동작하는 AI 코딩 에이전트
...
OpenHarness이 도움이 되었다면 스타를 눌러 주세요. ⭐
...
<p align="center">
  <em>방문해 주셔서 감사합니다 ✨ OpenHarness!</em><br><br>
